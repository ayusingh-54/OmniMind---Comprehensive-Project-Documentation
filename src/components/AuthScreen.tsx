...
const FEATURE_HIGHLIGHTS = [
    {
        ...
        desc: "RAG-powered private knowledge engine that helps AI deeply understand your documents, notes, and conversation history."
    },
    {
        icon: Shield,
        title: "Edge Privacy Computing",
        desc: "Data processed on local edge nodes, enabling trusted distributed inference through Parallax protocol."
    },
    {
        icon: Zap,
        title: "Intent-Driven Collaboration",
        desc: "LLM-based intent recognition engine, enabling Agents to shift from 'passive response' to 'active collaboration'."
    }
];
...
export const AuthScreen: React.FC<AuthScreenProps> = ({ onAuthenticated, error }) => {
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        if (runBlockingValidation()) {
            setLoading(false);
            setMessage('Please fix the form errors first');
            setMessageTone('error');
        ...
        try {
            if (isRegisterMode) {
                ...
                setMessageTone('success');
                setMessage('Login successful, entering chat room...');
            }
            await onAuthenticated();
        } catch (err: any) {
            const status = err?.status as number | undefined;
            const raw = (err?.message as string) || '';
            const reason = raw || (status ? `Request failed (${status})` : 'Request failed');
            let friendly: string;

            if (isRegisterMode) {
                if (status === 409) {
                    friendly = 'Email already registered, please login or use a different email';
                } else if (status === 400) {
                    friendly = raw || 'Registration info incomplete or password too short';
                } else {
                    friendly = `Registration failed: ${reason}`;
                }
            } else {
                if (status === 401) {
                    friendly = 'Email or password is incorrect';
                } else {
                    friendly = `Login failed: ${reason}`;
                }
            }
    ...
    const fieldErrors = useMemo(() => {
        ...
        if (isRegisterMode && touched.name) {
            if (!name.trim()) {
                ...
                next.name = 'At least 2 characters';
            }
        }
    ...
    return (
        <div className="auth-screen">
            ...
            <div className="auth-container">
                ...
                >
                    ...
                    <p className="brand-desc">
                        Transform every conversation into an intelligent workspace.
                        Integrate <b>long-term memory</b> and <b>private knowledge base</b>, making AI your powerful companion for life and work.
                    </p>
                </motion.div>
                        ...
                        <div className="auth-card">
                            <div className="card-header">
                                ...
                                <p className="auth-subtitle">
                                    {isRegisterMode ? 'Register to join the Parallax collaboration network' : 'Login to connect your agent workspace'}
                                </p>
                            </div>
                            ...
                            <div className="auth-footer">
                                ...
                                <button onClick={() => handleModeChange(isRegisterMode ? 'login' : 'register')}>
                                    {isRegisterMode ? 'Login' : 'Register Free'}
                                </button>
                            </div>
            ...
            /* Flowing background animation - light blue-purple silk effect */
            .background-gradient {
                position: fixed;
                top: 0;
                left: 0;
            ...
            /* Large screen adaptation - starts adjusting below 1200px */
            ...
            /* Tablet landscape & small laptop - below 1024px */
