...
export const MessageInput: React.FC = () => {
    ...
    const handleFileSelect = async (evt: React.ChangeEvent<HTMLInputElement>) => {
        ...
        if (!ALL_ALLOWED_EXTENSIONS.includes(ext)) {
            toast.error('Only text files and documents are supported (.txt, .md, .pdf, .docx, .pptx, etc.)');
            return;
        }
        ...
        if (file.size > maxSize) {
            toast.error(`File size cannot exceed ${isBinary ? '10MB' : '1MB'}`);
            return;
        }
        ...
        try {
            if (isBinary) {
                ...
                if (ext === '.html' || ext === '.xml') {
                    ...
                    if (!content.trim()) {
                        toast.error('No valid text content extracted from HTML file');
                        return;
                    }
                }
    ...
    const handleSend = async () => {
        ...
        // Use exact match: @username must be followed by space, punctuation, or end
            ...
            if (attachedFile) {
                if (uploadToRag) {
                    ...
                    try {
                        ...
                        const fileIndicator = `📎 [Attachment: ${attachedFile.name}] (Added to knowledge base)`;
                        messageContent = messageContent
                            ? `${messageContent}\n\n${fileIndicator}`
                            : fileIndicator;

                        toast.success(`File added to knowledge base (${uploadRes.chunksCreated} chunks)`);
                    } catch (uploadErr) {
                    ...
                    // Add visual indicator in message
                    const fileIndicator = `📎 [Attachment: ${attachedFile.name}]`;
                    messageContent = messageContent
                        ? `${messageContent}\n\n${fileIndicator}`
                        : fileIndicator;

                    toast.success(`File added as attachment`);
                }
            }
    ...
    return (
                        ...
                        >
                            <div className="reply-preview-content">
                                ...
                                <div className="reply-info">
                                    <span className="reply-label">Reply to {state.users.find(u => u.id === state.replyingTo?.senderId)?.name}</span>
                    ...
                    {attachedFile && (
                        ...
                        >
                            ...
                            <div className="file-preview-actions">
                                <label className="rag-toggle" title={uploadToRag ? 'Will be added to knowledge base' : 'Share as attachment only'}>
                                    <input
                                        type="checkbox"
                                        checked={uploadToRag}
                                    ...
                                    <span className="rag-toggle-label">{uploadToRag ? 'Add to knowledge base' : 'Share only'}</span>
                                </label>
                                <button onClick={removeAttachedFile} className="remove-file-btn" title="Remove file">
                                    <X size={16} />
                                </button>
                            </div>
                        </motion.div>
        ...
        .input-bar {
          ...
          border-radius: 28px; /* Large rounded corners, close to capsule shape */
          padding: 6px 8px 6px 12px;
          box-shadow: 
            0 4px 6px -1px rgba(0, 0, 0, 0.05),
            0 10px 30px -5px rgba(0, 0, 0, 0.08),
            0 0 0 1px rgba(0, 0, 0, 0.03); /* Very thin inner stroke to replace border */
          backdrop-filter: blur(24px);
          -webkit-backdrop-filter: blur(24px);
          transition: all 0.3s cubic-bezier(0.2, 0.8, 0.2, 1);
          position: relative;
          margin-bottom: 8px; /* Bottom padding for floating effect */
        }
        ...
        .input-bar.focused {
          ...
          box-shadow: 
            ...
            0 0 0 2px rgba(37, 99, 235, 0.1); /* Focus glow effect */
        ...
        /* Hide file preview border to make it more card-like */
        ...
        .send-btn {
          background: var(--accent-primary); /* Solid color is more premium than gradient */
          color: white;
          width: 40px;
          height: 40px;
          border-radius: 50%;
