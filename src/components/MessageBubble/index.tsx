...
export const MessageBubble: React.FC<MessageBubbleProps> = React.memo(({ message, isOwnMessage, showAvatar }) => {
    ...
    const handleAskAI = async (agent: Agent) => {
        ...
        // Get the user corresponding to the agent
        ...
        // Build question message: quote original message and @ AI
        const truncatedContent = message.content.length > 100
            ? message.content.slice(0, 100) + '...'
            : message.content;
        const askContent = `@${agentUser.name} Please explain or comment on this message: "${truncatedContent}"`;
        ...
        try {
            ...
            toast.success(`Requested ${agent.name} to reply`);
        } catch (err) {
    ...
    const handleDeleteConfirm = async () => {
        ...
        try {
            ...
            // Support cascade delete: backend returns deletedMessageIds array
    ...
    const handleMouseLeave = () => {
        clearHoverTimeout();
        // Don't hide actions when delete confirm or AgentSelector is open
