            ...
            <div className="attachment-info">
                ...
                <div className="attachment-meta">
                    ...
                    {attachment.uploadedToRag && !isDeleted && (
                        <span className="attachment-rag-badge" title={`Added to knowledge base (${attachment.chunksCreated || 0} chunks)`}>
                            <Database size={10} />
                            <span>Knowledge Base</span>
                        </span>
                    )}
                    {isDeleted && (
                        <span className="attachment-rag-deleted" title="Removed from knowledge base">
                            <span>Removed</span>
                        </span>
                    )}
                </div>
 ...
 * Matches patterns like: 📎 [Attachment: filename.ext] or 📎 [附件: filename.ext] (已添加到知识库)
 */
export function stripAttachmentText(content: string): string {
    // Remove attachment indicator lines
    return content
        .replace(/📎\s*\[(Attachment|附件):\s*[^\]]+\]\s*(\([^)]*\))?\s*/g, '')
        .trim();
}
